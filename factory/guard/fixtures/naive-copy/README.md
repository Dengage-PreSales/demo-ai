...
| `table-allowlist` | `sendDeviceEvent` targeting a variable rather than a literal | `cantuCatalog.js`, `wishlist.js`, `eventModal.js` |
...
The two modules worth understanding rather than skimming are `cantuCatalog.js`
and `eventModal.js`.

`cantuCatalog.js` makes no `ec:` call and names none of the five standard
