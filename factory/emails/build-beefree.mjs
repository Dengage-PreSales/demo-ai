...
export function buildBeefree(slug, snippets) {
    ...
    const template = beefreeAbandonedCart({
        ...
        storeUrl,
        /* THERE IS NO unsubscribe.html. See build-emails.mjs: the nearest real surface
           is the account overlay, and the link says what it does rather than claiming
           to unsubscribe. */
        unsubscribe: demoLink(storeUrl, 'account'),
        symbol: locale.currencySymbol || '',
        currency: locale.currency || '',
        /* The demo's own category structure, scraped from the prospect's site, so the
           nav in the email is their taxonomy rather than a made up one. */
        categories: config.categories || [],
        /* Drawn by make-hero.mjs from this demo's brand colour. Referenced only when it
           is actually on disk: a missing image in an email is worse than no image, and
           the hero is generated by a separate command that a build is allowed to skip. */
        heroImage: existsSync(join(ROOT, 'demos', slug, 'images', 'email-hero.jpg'))
            ? storeUrl + 'images/email-hero.jpg'
            : '',
        snippets: snippets || {
